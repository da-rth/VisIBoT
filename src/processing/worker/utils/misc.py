...
def filter_out_vt_keywords(s):
    return len(s) < 3 or any(x in s for x in IGNORE_VT_KEYWORDS)


def get_top_malware_keyword(scans):
    keywords = []
    strip_chars = "!@#$%^&*()[]{};:,./<>?\\|`~-=_+"

    for scan in scans.values():
        result = scan.get('result', None)

        if not result:
            continue

        result = result.translate({ord(c): " " for c in strip_chars})
        keywords += [s.lower() for s in result.split(" ") if s and not filter_out_vt_keywords(s.lower())]

    return most_common(keywords)
...
def valid_tld(domain_or_url):
    tlde = tld_extractor(domain_or_url)
...
def validate_url(url):
    ...
    """
    # ignore if URL has no path (e.g. http://website.com(/))
    if url.count('/') < 3 or (url.count('/') == 3 and url.endswith('/')):
        return None

    # ignore common xml schema urls
    if "schema" in url.lower():
        return None
